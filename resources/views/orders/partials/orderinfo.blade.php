...
<div class="card-body">
    {{--<h6 class="heading-small text-muted mb-4">{{ __('Shop information') }}</h6>--}}
    ...
    <ul id="order-items">
     ...
     @if (config('app.isft'))
         <h6 class="heading-small text-muted mb-1">{{ __('Client Information') }}</h6>
         <div class="pl-lg-4">
     ...
     <h5>{{__("Coupon Price")}}: {{$order->coupon_price}}</h5>
     <h5>{{__("Discount Price")}}: {{$order->discount}}</h5>
     <h5>{{__("Grand Price")}}: {{$order->order_price-$order->coupon_price-$order->discount}}</h5>
     @if(config('app.isft'))
     <h4>{{ __("Delivery") }}: @money( $order->delivery_price, $currency,$convert)</h4>
     @endif
     <hr />
     <h3>{{ __("TOTAL") }}: {{sprintf('%0.2f', $order->total_price-$order->discount)}}</h3>
     <hr />
